 ...
 * });
 */
export const useUserAppBreadcrumb = (config: BreadcrumbConfig) => {
  const context = useContext(LayoutContext);

  if (!context) {
    throw new Error("useBreadcrumb must be used within a AppLayout");
  }
          ...
          {config.items.map((item, index) => {
            ...
            return (
              <div key={index} className="flex items-center gap-2">
                <BreadcrumbItem>
                  {isCurrentPage ? (
                    <BreadcrumbPage className="text-[48px] font-medium text-[#1D2433] first-letter:uppercase">
                      {item.label}
                    </BreadcrumbPage>
                  ) : (
                    <Link
