...
const UserSidebar = () => {
  ...
  return (
    <aside
      className={cn(
        "font-inter relative z-50 flex h-screen flex-col border-r border-[#ECEFF3] bg-[#F6F8FA] transition-all duration-300",
                    ...
                    {link.subLinks.map((subLink, subIndex: number) => {
                      ...
                      const LinkContent = (
                        <NavLink
                          to={subLink.path}
                          className={cn(
                            "group flex w-full cursor-pointer items-center gap-2 rounded-md border-transparent px-3 py-[10px] text-sm text-[#99A0AE] transition-all duration-200",
                            isActive
                              ? "bg-white font-medium text-[#079455] shadow-[0px_1px_2px_0px_#E4E5E73D]"
